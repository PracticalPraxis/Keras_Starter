...
#These reviews themselves have been reduced to collections of integers, where each word of the review is represented by a number e.g. 'great' as 147 and 'fly' as 861
#So a review stating "This movie was so great it made me fly" would look like this in our dataset: [4, 1, 67, 998, 147, 76, 98, 32, 861] ([] used for neatness and not necessarily to present an array)
...
#Splitting the loaded data into separate categories for training and testing (we won't be using the test data, but it still needs to be loaded separately from the training data)
...
#we translate the [4, 1, 99] list into a format that is intelligible to our model by creating a vector of N dimensions (where N is the highest number presented in our dataset)
...
#Sequential is the most basic kind of overall structure for ML models, where each layer does its job fully, before passing the complete output to the next layer
#This is not the case for the more advanced models out there, which incorporate complex structures with non-sequential communication between layers
...
#In this case we're using the rmsprop optimizer which keeps a moving average over the root mean squared gradients (rms) which the gradient at use gets divided by
...
#i.e. we're sorting the review into positive or negative, so we need a loss measurement sensitive to binary classification and probability distribution (hence, crossentropy)
...
#First we feed it our actual data, the words of the review (partial_x_train) and the sentiment score for the review (partial_y_train)
#The model will run through all the words and labels it gets, and learn the score associated with certain reviews
#It will then be given reviews it does not know the sentiment score of (our validation set) and try to guess their sentiment score
